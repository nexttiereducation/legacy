(function () {
    angular.module('student-detail', ['student'])
    ...
    function StudentDetailManager($q, API, UrlHelper, StudentModel) {
        ...
        function getFollowedSchools(studentId) {
            return API.$get(UrlHelper.peer.institutionTracker(studentId), {hideLoader: true}).then(function (response) {
        ...
        function getGroupsStudentBelongsTo(studentId) {
            ...
            return API.$get(UrlHelper.stakeholder.groupsStudentBelongsTo(counselorId, studentId)).then(function (response) {
        ...
        function getStudentDetail(studentIds, next, deferred) {
            ...
            API.$get(next || UrlHelper.stakeholder.studentDetail(), data).then(function (response) {
                    StudentModel.studentsDetails.push.apply(StudentModel.studentsDetails, response.data.results);
        ...
        function getStudentTaskList(studentId, next, deferred, taskList) {
            ...
            API.$get(next || UrlHelper.peer.taskList(studentId))
                .then(function (response) {
                    ...
                    if (response.data.next) {
                        getStudentTaskList(studentId, response.data.next, localDeferred, taskList);
                    } else {
                        var taskInfo = countTaskListInformation(taskList, studentId);
