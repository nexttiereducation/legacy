(function() {
    ...
    StakeholderAuth.$inject = ['$rootScope', '$q', 'API', '$location',
        ...
        '$cookies', 'Ambassador', 'NotificationManager',
        'HighSchoolManager'
    ];

    function StakeholderAuth($rootScope, $q, API, $location, UrlHelper,
        localStorageService, Facebook, Track, $cookies, Ambassador,
        NotificationManager, HighSchoolManager) {
        var baseStakeholder = {
            loggedIn: false,
            anonymous: false,
            firstName: 'Guest'
        ...
        return {
            ...
            basicRegistration: function(data) { return basicRegistration(data); },
            ...
            facebookCheckLoginStatus: function() { return facebookCheckLoginStatus(); },
            facebookLogin: function() { return facebookLogin(); },
            facebookRegistration: function(data) { return facebookRegistration(data); },
            getAlternateContact: getAlternateContact,
            getAuthToken: getAuthToken,
            getPreferences: getPreferences,
            getStakeholder: function() { return getStakeholder(); },
            getStakeholderInformation: getStakeholderInformation,
            getTrackingType: function() { return getTrackingType(); },
            getUser: function() { return getStakeholder(); },
            getUserId: getUserId,
            ghostLogin: function(data) { return ghostLogin(data); },
            ghostLogout: function(data) { return ghostLogout(data); },
            ...
            isUpperClassman: function() { return isUpperClassman(); },
            logout: function() { return logout(); },
            needsLogin: needsLogin,
            parseDispalyStakeholderType: function(type) { return parseDispalyStakeholderType(type); },
            parseDisplayName: function(fn, ln, email) { return parseDisplayName(fn, ln, email); },
            parseTrackingType: function(stakeholder_type) { return parseTrackingType(stakeholder_type); },
            parseUserPhoto: function(photo, type) { return parseUserPhoto(photo, type); },
            resendConfirmation: function(data) { return resendConfirmation(data); },
            resetPassword: function(data) { return resetPassword(data); },
            saveStakeholderData: saveStakeholderData,
            setSession: function(token, id) { return setSession(token, id); },
            setupUser: function() { return setupUser(); },
            updateAlternateContact: updateAlternateContact,
            updateStakeholder: function() { return updateStakeholder(); },
            updateStakeholderACT: updateStakeholderACT,
            updateStakeholderDetails: updateStakeholderDetails,
            updateStakeholderInstitutions: function(data) { return updateStakeholderInstitutions(data); },
        ...
        function isUpperClassman() {
            return (/senior/.test(user.phase.toLowerCase()) || /junior/.test(
                user.phase.toLowerCase()));
        }
     ...
     */
        ...
        function getFullNameOri() {
            return (user.firstName || user.lastName ? (user.firstName || '') +
                ' ' + (user.lastName || '') : (user.email).replace(/@.*/, '')).trim();
        }
        ...
        function parseDisplayName(fn, ln, email) {
            return (fn || ln ? (fn || '') + ' ' + (ln || '') : (email).replace(/@.*/, '')).trim();
        }
        /*	Exposed as 'parseTrackingType'
         */
        function parseTrackingType(stakeholder_type) {
            return (user.isStudent) ? 'Students' : 'Mentors';
        }
        ...
        function getPreferences() {
            return API.$get(UrlHelper.stakeholder.preferences()).then(
                function(response) {
        ...
        function getAlternateContact() {
            return API.$get(UrlHelper.stakeholder.alternateContact()).then(
                function(response) {
     ...
     */
        function getStakeholderInformation() {
            return API.$get(UrlHelper.stakeholder.details()).then(function(
                response) {
                ...
                user.hasConnections = (data.adult_connections.length >
                    0 || data.student_connections.length > 0);
                ...
                alert(
                    'Could not retrieve user information, please reload the page.'
                );
            });
        }
            ...
            if (user.entitlements) {
                ...
                for (var idx in user.entitlements) {
                    if (entitlements[idx].name.toUpperCase() == actionName.toUpperCase()) {
                        ...
                        if (entitlements[idx].resource && resource.toUpperCase() ==
                            entitlements[idx].resource.toUpperCase()) {
                            return true;
                        }
                    }
                }
        ...
        function getEntitlements() {
            ...
            return API.$getPaged(UrlHelper.stakeholder.entitlements())
     ...
     */
        ...
        function parseDispalyStakeholderType(stakeholderType) {
     ...
     */
        function basicLogin(data) {
            ...
            return API.$post(UrlHelper.stakeholder.signIn(), data).then(
                function(response) {
                    return setSession(response.data.token, response.data.id)
                        .then(function() {
                            Track.event(
                                'logged_in_with_basic_login');
                            return getStakeholderInformation().then(
                                function(response) {
                                    return response;
                                });
                        });
        ...
        function ghostLogin(login) {
            var url = UrlHelper.app.ghostLogin();
            var oldToken = localStorageService.get('authToken');
            var ghostData = { 'token': oldToken, 'email': user.email, 'id': user.id };
            API.$post(url, login).then(function(response) {
                // destroySession must happen after ghost login call
                destroySession();
                user.email = login.email;
                localStorageService.set('isGhost', true);
                localStorageService.set('ghostData', JSON.stringify(
                    ghostData));
                return setSession(response.data.token, response.data.id)
                    .then(function() {
                        // Eventually want to add tracking for the ghost login
                        // Track.event('logged_in_with_basic_login');
                        return getStakeholderInformation().then(
                            function(response) {
        ...
        function ghostLogout() {
            ...
            return setSession(ghostData.token, ghostData.id)
                .then(function() {
                    // Will want to set tracking for ghost logout eventually.
                    //Track.event('logged_in_with_basic_login');
                    return getStakeholderInformation().then(function(
                        response) {
        ...
        function basicRegistration(data) {
            if (user.anonymous) {
                ...
                return API.$post(UrlHelper.stakeholder.register(), data)
                    .then(function(response) {
                        Track.signUp(response.data, false, data);
                        Track.event('user_created', {
                            'user type': 'registered',
                            'user type 2': data.stakeholder_type,
                            'referring ambassador': Ambassador.get() ||
                                'none'
                        });
                        return response;
                    });
        ...
        function basicAnonUpgrade(data) {
            ...
            return API.$post(UrlHelper.stakeholder.upgradeAnonymous(), data)
                .then(function() {
                    Track.signUp(StakeholderAuth.id, false, data);
                    Track.event('user_created', {
                        'user type': 'registered',
                        'user type 2': data.stakeholder_type,
                        'referring ambassador': Ambassador.get() ||
                    ...
                    alert(
                        'I\'m sorry, we were unable to upgrade your account.  Please try again.'
                    );
                });
        }
        ...
        function facebookRegistration(userData) {
            return facebookHandshake().then(function(response) {
                if (response.status == 'connected') {
                    ...
                    return facebookConfirmation(userData).then(
                        function(response) {
                            Track.event('user_created', {
                                'user type': 'registered',
                            ...
                            Track.signUp(response.data.id, true,
                                userData);
                        });
                }
                return 'NO_FB_CONNECTION';
     ...
     */
        ...
        function resendConfirmation(data) {
            return API.$post(UrlHelper.stakeholder.resendConfirmation(), data);
        }

        function resetPassword(data) {
            return API.$post(UrlHelper.stakeholder.forgotPassword(), data);
        }

        function createAlternateContact(data) {
            return API.$post(UrlHelper.stakeholder.alternateContact(), data)
                .then(function(response) {
        ...
        function updateAlternateContact(data, id) {
            if (!data.value || data.value.length === 0) {
                return API.$delete(UrlHelper.stakeholder.updateAlternateContact(id))
                    .then(function(response) {
                        return response.data;
                    });
            } else {
                return API.$patch(UrlHelper.stakeholder.updateAlternateContact(id), data)
                    .then(function(response) {
                        return response.data;
                    });
            }
        ...
        function updateStakeholderDetails(data) {
            return API.$patch(UrlHelper.stakeholder.details(), data)
                .then(function(response) {
                    return response;
                });
        }
        ...
        function updateStakeholderACT(act_scores, act_id) {
            return API.$patch(UrlHelper.stakeholder.updateActScores(act_id), act_scores)
                .then(function(response) {
                    return response;
                });
        }
        ...
        function updateStakeholderSAT(sat_scores, sat_id) {
            return API.$patch(UrlHelper.stakeholder.updateSatScores(sat_id), sat_scores)
                .then(function(response) {
                    return response;
                });
        }
        ...
        function deleteStakeholderACT(act_id) {
            return API.$delete(UrlHelper.stakeholder.updateActScores(act_id))
                .then(function(response) {
                    return response;
                });
        }
        ...
        function deleteStakeholderSAT(sat_id) {
            return API.$delete(UrlHelper.stakeholder.updateSatScores(sat_id))
                .then(function(response) {
                    return response;
                });
        }
        ...
        function createStakeholderACT(act_test) {
            return API.$post(UrlHelper.stakeholder.makeActScores(), act_test)
                .then(function(response) {
                    return response;
                });
        }
        ...
        function createStakeholderSAT(sat_scores) {
            return API.$post(UrlHelper.stakeholder.makeSatScores(), sat_scores)
                .then(function(response) {
                    return response;
                });
        }
        ...
        function deleteAccount() {
            return API.$delete(UrlHelper.stakeholder.deleteAccount())
                .then(function(response) {
                    return response;
                })
        ...
        function changePassword(data) {
            return API.$post(UrlHelper.stakeholder.changePassword(), data)
                .then(function(response) {
                    return response;
                });
        }
